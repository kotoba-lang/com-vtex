(ns vtex.main
  "Kotodama WASM entrypoint for the Vtex clean-room actor (L4) — Clojure port.
  ...
  facts (`vtex.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "vtex_pro"
   ...
   {:entity "Customer"  :plural "customers"  :id-prefix "vtex_cus"
    :fields [:email :firstName :lastName :phone] :required [:email :firstName]
    :coerce {}                           :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "vtex_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"vtex.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "vtex-compat" :tier tier :entities entities} 200])
